...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func postTweet(tweetString: String, success: @escaping () -> (), failure: @escaping (Error) -> ())
    {
        ...
        TwitterAPICaller.client?.post(tweetUrl, parameters: ["status": tweetString], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
    ...
    {
        let createFavUrl = "https://api.twitter.com/1.1/favorites/create.json"
        TwitterAPICaller.client?.post(createFavUrl, parameters: ["id": tweetID], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
    ...
    {
        let destroyFavUrl = "https://api.twitter.com/1.1/favorites/destroy.json"
        TwitterAPICaller.client?.post(destroyFavUrl, parameters: ["id": tweetID], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
    ...
    func retweet(tweetID: Int, success: @escaping ()->(),failure: @escaping (Error)->())
    {
        ...
        //Note: when there is a :attribute in the url, it normally wants you to replace it with something.
        //eg, in the retweetUrl, it was "https:xxxxxxxx/:id.json". We need to replace the :id with the tweetID in the param.
    ...
    func unretweet(tweetID: Int, success: @escaping ()->(),failure: @escaping (Error)->())
    {
        ...
        //Note: when there is a :attribute in the url, it normally wants you to replace it with something.
        //eg, in the retweetUrl, it was "https:xxxxxxxx/:id.json". We need to replace the :id with the tweetID in the param.
        TwitterAPICaller.client?.post(unretweetUrl, parameters: ["id": tweetID], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
